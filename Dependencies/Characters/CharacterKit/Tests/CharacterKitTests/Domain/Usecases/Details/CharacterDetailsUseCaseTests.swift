...
class CharacterDetailsUseCaseTests: XCTestCase {
    ...
    func testCharacterDetailsUseCase_characterDetails_shouldLoadWithSuccess() async {
        ...
        do {
            // When
            let publisher: AnyPublisher<CharacterImpl, Error> = try await sut.characterDetails(params: [:])
            publisher
                .sink { completion in
            ...
            print("Exception in testCharacterDetailsUseCase_loadingCharacters_shouldLoadWithSuccess = \(error)")
        }
    }
}
